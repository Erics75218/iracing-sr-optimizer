...
export default async function OvalPage({ searchParams }: Props) {
  ...
  const ovalSeries = getSeriesByCategory(effectiveSeason, 1);
  const isMock = !season;
  ...
  const seriesForGoldenPath =
    goldenIds == null || goldenIds.length === 0
      ? ovalSeries ?? []
      : (ovalSeries ?? []).filter((s) => goldenIds.includes(s.series_id));
  ...
  return (
    <div className="space-y-6">
      <Suspense fallback={null}>
        <GoldenPathRestore basePath="/dashboard/oval" disciplineKey="oval" />
      </Suspense>
      ...
      <p className="text-muted-foreground">
        {seriesNotFound
          ...
          : selectedSeries
            ...
            : "Select an Oval series for this week's track and schedule, or browse SR-friendly recommendations below."}
      </p>
      ...
      {!selectedSeries && (ovalSeries?.length ?? 0) > 0 && (
        <section className="rounded-lg border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-900 dark:bg-amber-950/20">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            One race per week that gives the highest potential corners in Oval. Run these for maximum SR gain with 0×. Uncheck series you want to omit (e.g. very long races). Click a series below to see its schedule and track data instead.
          </p>
          <GoldenPathSeriesSelector
            disciplineSeries={ovalSeries ?? []}
            selectedGoldenIds={selectedGoldenIds}
            basePath="/dashboard/oval"
            disciplineKey="oval"
          />
          {goldenPath.length > 0 && (
      ...
      <section className="rounded-lg border p-4 bg-card">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {seriesFilter
            ...
            : "Oval races this season, ranked by length and lap count."}
        </p>
        <div className="mt-3">
