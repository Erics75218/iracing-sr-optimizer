...
export default async function SportsCarPage({ searchParams }: Props) {
  ...
  const sportsCarSeries = getSportsCarSeries(effectiveSeason);
  const isMock = !season;
  ...
  const seriesForGoldenPath =
    goldenIds == null || goldenIds.length === 0
      ? sportsCarSeries ?? []
      : (sportsCarSeries ?? []).filter((s) => goldenIds.includes(s.series_id));
  ...
  return (
    <div className="space-y-6">
      <Suspense fallback={null}>
        <GoldenPathRestore basePath="/dashboard/sports-car" disciplineKey="sports-car" />
      </Suspense>
      ...
      {!selectedSeries && (sportsCarSeries?.length ?? 0) > 0 && (
        <section className="rounded-lg border-2 border-primary/30 bg-primary/5 p-4 dark:border-primary/40 dark:bg-primary/10">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            One race per week that gives the highest potential corners in Sports Car. Run these for maximum SR gain with 0×. Uncheck series you want to omit (e.g. very long races). Click a series below to see its schedule and track data instead.
          </p>
          <GoldenPathSeriesSelector
            disciplineSeries={sportsCarSeries ?? []}
            selectedGoldenIds={selectedGoldenIds}
            basePath="/dashboard/sports-car"
            disciplineKey="sports-car"
          />
          {goldenPath.length > 0 && (
