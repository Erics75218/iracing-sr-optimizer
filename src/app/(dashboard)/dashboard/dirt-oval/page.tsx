...
const DIRT_OVAL_OPTIONS = { categoryId: 3 as const, limit: 20 };

type Props = { searchParams: Promise<{ series?: string; golden?: string | string[] }> };

export default async function DirtOvalPage({ searchParams }: Props) {
  ...
  const dirtOvalSeries = getSeriesByCategory(effectiveSeason, 3);
  const isMock = !season;
  ...
  const seriesForGoldenPath =
    goldenIds == null || goldenIds.length === 0
      ? dirtOvalSeries ?? []
      : (dirtOvalSeries ?? []).filter((s) => goldenIds.includes(s.series_id));
  ...
  return (
    <div className="space-y-6">
      <Suspense fallback={null}>
        <GoldenPathRestore basePath="/dashboard/dirt-oval" disciplineKey="dirt-oval" />
      </Suspense>
      ...
      <p className="text-muted-foreground">
        {seriesNotFound
          ...
          : selectedSeries
            ...
            : "Select a Dirt Oval series for this week's track and schedule, or browse SR-friendly recommendations below."}
      </p>

      {!selectedSeries && (dirtOvalSeries?.length ?? 0) > 0 && (
        <section className="rounded-lg border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-900 dark:bg-amber-950/20">
          ...
          <GoldenPathSeriesSelector
            disciplineSeries={dirtOvalSeries ?? []}
            selectedGoldenIds={selectedGoldenIds}
            basePath="/dashboard/dirt-oval"
            disciplineKey="dirt-oval"
          />
          {goldenPath.length > 0 && (
      ...
      <section className="rounded-lg border p-4 bg-card">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {seriesFilter
            ...
            : "Dirt Oval races this season, ranked by length and lap count."}
        </p>
        <div className="mt-3">
